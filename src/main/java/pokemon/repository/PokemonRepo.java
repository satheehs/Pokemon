...
public class PokemonRepo {
    ...
    PriorityQueue<Pokemon> maxQueue = new PriorityQueue<>((t1, t2) -> t2.getArge() - t1.getArge());
    PriorityQueue<Pokemon> minQueue = new PriorityQueue<>(Comparator.comparingInt(Pokemon::getArge));
    ...
    public BasePokemon getPokemon_id(String id) {
    ...
    public String addPokemon(BasePokemon basePokemon) {
        String id = null;
        if (basePokemon != null) {
    ...
    public Pokemon maxArgePokemon() {
        Optional<Pokemon> max = pokemonRepo.values().parallelStream().max((a, b) -> b.getArge() - a.getArge());
        if (max.isPresent()) {
            return max.get();
        } else {
            return null;
    ...
    public Pokemon minArgePokemon() {
        Optional<Pokemon> min = pokemonRepo.values().parallelStream().max(Comparator.comparingInt(Pokemon::getArge));
        if (min.isPresent()) {
            return min.get();
        } else {
            return null;
